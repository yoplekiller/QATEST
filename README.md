...
DB_HOST=localhost                            # SQL 테스트용
DB_PORT=3306                                 # SQL 테스트용
DB_USER=root                                 # SQL 테스트용
DB_PASSWORD=your_db_password                 # SQL 테스트용
DB_NAME=tmdb_test                            # SQL 테스트용
```
...
| 테스트 | 검증 내용 |
|--------|-----------|
| `test_ui_login` (3) | 유효/무효 로그인, 빈 입력 처리, 페이지 요소 확인 |
| `test_ui_search` (7) | 정상 검색, 빈 검색, 특수문자, 결과 클릭 |
| `test_ui_cart` (1) | 장바구니 페이지 접근 확인 |
| `test_ui_add_goods` (1) | 상품 검색 → 수량 조절 (2회 증가, 1회 감소) → 장바구니 담기 |
| `test_add_goods_to_cart` (1) | 로그인 → 상품 검색 → 수량 조절 → 장바구니 담기 플로우 |
| `test_ui_goods_add_flow` (1) | 로그인 → 검색 → 상품 추가 → 장바구니 이동 E2E |
| `test_cart_management` (2) | 여러 상품 장바구니 담기, 장바구니 상품 삭제 |
| `test_blank_search` (1) | 빈 검색어 처리 확인 |
| `test_ui_sort_button` (4) | 추천순/신상품순/낮은가격순/높은가격순 정렬 |
| `test_ui_quantity` (1) | ⚠️ skip - 비로그인 장바구니 팝업 미지원 |
| `test_invalid_search` (1) | ⚠️ skip - 컬리 검색 결과 없음 메시지 UI 변경 |
...
| 테스트 | 검증 내용 |
|--------|-----------|
| `test_no_null_movie_ids` | movie_id NULL 없음 검증 |
| `test_no_duplicate_movie_ids` | 중복 ID 없음 검증 |
| `test_vote_average_range` | 평점 0~10 범위 검증 |
| `test_release_date_format` | 날짜 형식 (YYYY-MM-DD) 검증 |
| `test_api_db_movie_count_match` | API 결과 수와 DB 행 수 일치 |
| `test_api_db_movie_title_match` | API 제목과 DB 제목 일치 |
| `test_api_db_vote_average_match` | API 평점과 DB 평점 일치 |
| `test_top_rated_movies_query` | ORDER BY 상위 평점 영화 조회 |
| `test_movies_by_popularity` | 인기도 기준 정렬 검증 |
| `test_movie_count_by_year` | GROUP BY 연도별 영화 수 집계 |

테스트 흐름: TMDB API 데이터 fetch → MySQL 저장 → SQL 쿼리로 데이터 무결성/정합성 검증
