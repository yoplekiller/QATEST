...
class TestLogin:
    ...
    **예상 결과:** 로그인 실패 메시지가 화면에 표시됨
    ...
    @pytest.mark.skip(
        reason=(
            "실사이트 조사로 확인됨: 로그인 폼에 봇 탐지/보안 계층이 새로 추가되어, 자동화된 "
            "로그인 시도는 '아이디/비밀번호 불일치' 안내 대신 '보안 인증 과정에서 오류가 "
            "발생하였습니다' 보안 차단 메시지를 받음(실제 자격증명 검증 단계까지 도달하지 "
            "못함). 이 보안 계층을 우회하는 건 범위 밖 - 전용 테스트 계정/환경 없이는 이 "
            "케이스를 신뢰성 있게 자동화할 수 없음."
        )
    )
